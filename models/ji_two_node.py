...
def two_nodes_optimized(
    ...
    initial_skin_temp = 36.8,
    initial_core_temp = 36.49,
):
    ...
    # Calculate threshold value of body temperature t_body to be used for sweating model using initial value of alpha
    ...
    while n_simulation < length_time_simulation:
        ...
        d_t_sk = (
            (s_skin * body_surface_area) / (tc_sk * 60.0)
        ...
        m_bl = (skin_blood_flow_neutral + c_de * c_dil * t_cr_dil) / (1 + c_ce * c_str * t_sk_cons) # sbf
        ...
        m_rsw = (
            c_swe * c_sw * ((1 - alfa) * t_cr_sw + (alfa + a_cof) * t_sk_sw) * math.exp(t_sk_sw / 10.7)
            )
        ...
        r_e_a = 1 / (lr * f_a_cl * h_cc) # evaporative resistance of air layer in K*m2*atm/W
        r_total = r_e_cl + r_e_a # total evaporative resistance in K*m2*atm/W

        e_max = (math.exp(18.6686 - 4030.183 / (t_skin + 235.0)) - vapor_pressure) / r_total
        ...
        he_n = 1/(r_total) # coefficient of the evaporative heat transfer (inverse of the total evaporative heat resistance; W/(K*m2*Pa))
        wettedness_dif = 1 / (2 + 2.46 * he_n)
        wp = wettedness_dif + (1 - wettedness_dif) * p_rsw # skin wetness (between 0.06 and 1)
        ...
        met_shivering = c_she * (cof_scs * t_cr_sh * t_sk_cons + cof_sc * t_cr_sh + cof_ss * t_sk_cons)
